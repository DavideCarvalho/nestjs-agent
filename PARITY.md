...
| Structured output | yes | yes | Adonis delivers the value as a synthetic tool-call ROW rather than on the message and the stream. Its loop feeds tool results back as an EMPTY `user` message, so the restatement prompt takes the last user message WITH CONTENT — scanning for the last `user` role handed the pass a blank question on every turn that called a tool. |
| Ask the user a structured question | yes — configured intake + model-callable `ask` | yes | Both surfaces write ONE persisted shape and park on the approval signal, so a consumer cannot tell which asked. Adonis has no `setMessageToolResults`, so settled answers live on the tool-call row; the transcript still carries the full round-trip. |
