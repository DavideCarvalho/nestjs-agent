...
Every provider in this repo's own specs is an object literal of arrow functions, which needs no receiver — which is exactly why it went unseen. The regression test is a provider written the way a host writes one, with its storage on `this`.
