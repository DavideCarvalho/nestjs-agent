...
/** The mount default; matches `AgentDashboardModule.forRoot()`'s own default. */
const DEFAULT_BASE_PATH = '/ai-gateway';
 ...
 */
export function agentConsoleSessionUrl(basePath: string = DEFAULT_BASE_PATH): string {
  return `${normalizeBasePath(basePath)}/auth/session`;
}

/** Where the console SPA itself is served. */
export function agentConsoleUrl(basePath: string = DEFAULT_BASE_PATH): string {
  return normalizeBasePath(basePath) || '/';
}

export interface OpenConsoleOptions {
  /** Where the console is mounted. MUST match `AgentDashboardModule.forRoot({ basePath })`. */
  basePath?: string;
  /**
 ...
 */
export async function mintAgentConsoleSession(options: OpenConsoleOptions = {}): Promise<void> {
  const url = agentConsoleSessionUrl(options.basePath);
 ...
 */
export async function openAgentConsole(options: OpenConsoleOptions = {}): Promise<void> {
  await mintAgentConsoleSession(options);
  const target = agentConsoleUrl(options.basePath);
