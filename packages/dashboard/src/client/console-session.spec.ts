...
describe('agentConsoleSessionUrl', () => {
  it('derives the mint path from the default mount', () => {
    expect(agentConsoleSessionUrl()).toBe('/ai-gateway/auth/session');
    expect(agentConsoleUrl()).toBe('/ai-gateway');
  });
  ...
  it('follows a custom basePath', () => {
    ...
    expect(agentConsoleSessionUrl('/ops/ai')).toBe('/ops/ai/auth/session');
    expect(agentConsoleUrl('/ops/ai')).toBe('/ops/ai');
  });

  it('tolerates a missing leading slash and a trailing one', () => {
    expect(agentConsoleSessionUrl('ops/')).toBe('/ops/auth/session');
    expect(agentConsoleUrl('ops/')).toBe('/ops');
  });
});

describe('mintAgentConsoleSession', () => {
  it('posts with credentials so the Set-Cookie sticks', async () => {
    const fetchMock = vi.fn().mockResolvedValue(response());
    await mintAgentConsoleSession({ fetch: fetchMock });

    const [url, init] = firstCall(fetchMock);
    expect(url).toBe('/ai-gateway/auth/session');
  ...
  it('throws with the status when the console refuses', async () => {
    ...
    await expect(mintAgentConsoleSession({ fetch: fetchMock })).rejects.toBeInstanceOf(
      ConsoleSessionError,
    );
    await expect(mintAgentConsoleSession({ fetch: fetchMock })).rejects.toMatchObject({
      status: 401,
      url: '/ai-gateway/auth/session',
    });
  });
  ...
  it('reports a browser opaque redirect as an interception, not a success', async () => {
    ...
    await expect(mintAgentConsoleSession({ fetch: fetchMock })).rejects.toThrow(
...
describe('openAgentConsole', () => {
  ...
  it('navigates to the console after a successful mint', async () => {
    const fetchMock = vi.fn().mockResolvedValue(response());
    await openAgentConsole({ fetch: fetchMock, navigate });
  ...
  it('navigates to the custom mount', async () => {
    const fetchMock = vi.fn().mockResolvedValue(response());
    await openAgentConsole({ fetch: fetchMock, navigate, basePath: '/ops/ai' });

    expect(navigate).toHaveBeenCalledWith('/ops/ai');
  });

  it('does NOT navigate when the mint is refused', async () => {
    const fetchMock = vi.fn().mockResolvedValue(response({ status: 403 }));

    await expect(openAgentConsole({ fetch: fetchMock, navigate })).rejects.toBeInstanceOf(
      ConsoleSessionError,
    );
  ...
  it('does NOT navigate when the response was an intercepted redirect', async () => {
    ...
    await expect(openAgentConsole({ fetch: fetchMock, navigate })).rejects.toThrow(
      /answered with a redirect/,
    );
    expect(navigate).not.toHaveBeenCalled();
  });
