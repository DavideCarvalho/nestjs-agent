 ...
 * true`); `AgentRunWorkflow` injects it. Kept in its own file — `AgentDurableModule` provides
 * `AgentRunWorkflow` and `AgentRunWorkflow` injects this token, so declaring it in either of those
 * two files would make them import each other.
 */
