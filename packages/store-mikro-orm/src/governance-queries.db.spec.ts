...
describe('MikroOrmGovernanceQueries delegation edge (sqlite)', () => {
  let treeOrm: MikroORM;
  let treeQueries: MikroOrmGovernanceQueries;

  beforeAll(async () => {
    treeOrm = await MikroORM.init({
      driver: SqliteDriver,
      dbName: ':memory:',
      entities: agentEntities(),
      allowGlobalContext: true,
    });
    await ensureAgentSchema(treeOrm);
    treeQueries = new MikroOrmGovernanceQueries(treeOrm.em, new MikroOrmPricingStore(treeOrm.em));

    const em = treeOrm.em.fork();
    const thread = em.create(AgentThread, {
      id: 'thread-tree',
      actorRef: 'nora',
