...
- [#101](https://github.com/DavideCarvalho/nestjs-agent/pull/101) [`a60bd23`](https://github.com/DavideCarvalho/nestjs-agent/commit/a60bd2359bcdfa51c22fea60034635a0a5b3af41) Thanks [@DavideCarvalho](https://github.com/DavideCarvalho)! - The library now owns the memory table: `agent_memory`, plus `MikroOrmMemoryProvider` over it.
  ...
  **This reverses a deliberate omission, so here is why.** `agentManagedTables()` listed seven tables
  ...
  - **There is nothing for a host to relate to.** A skill row plausibly joins a host's own entities —
    ...
    fact, an opaque scope token and an origin. The scope is opaque _by construction_, so a `varchar`
